...
public class bookmark extends JFrame {
    ...
    public bookmark() {// 북마크 생성자
        ...
        MysqlConnection connectionFavorite = new MysqlConnection();
     ...
     // 세 번째 탭
        ...
        tabbedPane.add("친구 단어장", panel3);  // 탭 이름
        ...
        tabbedPane.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent e) {
                ...
                } else if (tabbedPane.getSelectedIndex() == 2) {  // 친구 단어장 탭이 선택된 경우
                    model3.setRowCount(0);  // 모든 행 제거
                    // 친구 목록 데이터를 불러와서 친구의 단어장 목록을 모델에 추가하는 로직
                    MysqlConnection connection3 = new MysqlConnection();
                    String[] friends = connection3.friendList(userId);
                    for (String friend : friends) {
                        String[] notes = connection3.noteList(friend);
